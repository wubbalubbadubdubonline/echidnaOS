...
void build_idt_entry(uint8_t vector, uint16_t selector, uint64_t offset, uint8_t type);

void build_pointer(void) {
	mem_store_w(IDT_pointer, 0x30F);	// idt length
	mem_store_q(IDT_pointer+2, IDT);	// idt start
	load_idt(IDT_pointer);				// load the IDT register
}

void build_idt_entry(uint8_t vector, uint16_t selector, uint64_t offset, uint8_t type) {
	mem_store_w((vector*16)+IDT, offset&0xFFFF);
	mem_store_w((vector*16)+IDT+2, selector);
	mem_store_b((vector*16)+IDT+5, type);
	mem_store_w((vector*16)+IDT+6, (offset&0xFFFF0000)/0x10000);
	mem_store_d((vector*16)+IDT+8, offset/0x100000000);
}
...
void create_IDT(void) {
	...
	uint64_t handler_simple_ptr_int = (uint64_t)handler_simple_ptr;
	uint64_t handler_code_ptr_int = (uint64_t)handler_code_ptr;
	uint64_t handler_irq_pic0_ptr_int = (uint64_t)handler_irq_pic0_ptr;
	uint64_t handler_irq_pic1_ptr_int = (uint64_t)handler_irq_pic1_ptr;
	uint64_t keyboard_isr_ptr_int = (uint64_t)keyboard_isr_ptr;
