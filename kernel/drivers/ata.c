...
uint8_t ide_print_error(uint32_t drive, uint8_t err) {
    ...
    kprint("- [");
    kprint((char *[]){"Primary", "Secondary"}[ide_devices[drive].channel]); // Use the channel as an index into the array
    kprint(",");
    kprint((char *[]){"Master", "Slave"}[ide_devices[drive].drive]); // Same as above, using the drive
    kprint("]\n");
    return err;
}
...
void ide_initialize(uint32_t BAR0, uint32_t BAR1, uint32_t BAR2, uint32_t BAR3, uint32_t BAR4) {
   ...
   for (i = 0; i < 4; i++)
      if (ide_devices[i].reserved == 1) {
        //kprint(" Found an ");
        //kprint((char *[]){"ATA", "ATAPI"}[ide_devices[i].type]);         /* Type */
        //kprint(" drive ");
        //int size = ide_devices[i].size / 2;
        //char buf[33];
        //int_to_ascii(size, buf);
        //kprint(buf);
        //kprint(" KiB ID: ");
        //memory_set_char(buf, 0, 33);
        //int_to_ascii(i, buf);
        //kprint(buf);
        //kprint("\n");
      }
